...
function EFFECT:Init(data)
    ...
    if !wpn.Suppressed and !wpn.FlashHidden then
        ...
        if particle then
            ...
            particle:SetDieTime(0.101)
            particle:SetStartAlpha(math.Rand(150, 200))
            particle:SetEndAlpha(0)
            particle:SetStartSize(math.Rand(9, 12)+pwpn:GetBurstCount()*0.2)
    ...
    for i = 1, quality do
        ...
        if particle then
            particle:SetVelocity(VectorRand() * (10+pwpn:GetBurstCount()) + addvel + Vector(0, 0, -10))
            particle:SetLifeTime(0)
            particle:SetDieTime(math.Rand(1, 1.5))
            particle:SetStartAlpha(math.Rand(35, 45)+pwpn:GetBurstCount()*0.5 * (4 - quality))
            particle:SetEndAlpha(30 * (4 - quality))
            particle:SetStartSize(3+pwpn:GetBurstCount()*0.5)
            particle:SetEndSize(2+pwpn:GetBurstCount()*0.5)
