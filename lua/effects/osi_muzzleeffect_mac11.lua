...
function EFFECT:Init(data)
    ...
    for i = 1, quality do
        ...
        if particle then
            particle:SetVelocity(VectorRand() * (4+pwpn:GetBurstCount()*0.5) + addvel + Vector(0, 0, 10))
            particle:SetLifeTime(0)
            particle:SetDieTime(math.Rand(1, 1.5))
            particle:SetStartAlpha(math.Rand(10, 15) * (4 - quality))
            particle:SetEndAlpha(4 * (4 - quality))
            particle:SetStartSize(0+pwpn:GetBurstCount()*0.2)
            particle:SetEndSize(3+pwpn:GetBurstCount()*0.2)
